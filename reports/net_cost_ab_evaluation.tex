...
\paragraph{A caution on reading the change column.} Two runs is not a sample. The
repository fix is not the only thing that differs between them in practice ---
the generator is not bit-reproducible across runs even at temperature zero, and
parallel merge order varies --- so part of any change is ordinary
run-to-run variation whose magnitude is unmeasured. The column is informative
about direction and rough scale, not a treatment effect estimate.
